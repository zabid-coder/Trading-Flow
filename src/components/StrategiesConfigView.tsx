...
import { STRATEGY_PRESETS } from "../engine/types";
            ...
            <div className="p-3 rounded-lg bg-[#141f33] border border-[#1e2d4a] flex items-center justify-between">
              <div>
                ...
                <span className="font-bold text-white text-sm">
                  {cfg.rbEnabled ? `STATE: ${st.rbState}` : "INACTIVE"}
                </span>
              </div>
              {st.rbHigh && st.rbLow && (
                <div className="text-right text-[11px]">
                  <span className="text-emerald-400 block">High: {st.rbHigh.toFixed(2)}</span>
                  <span className="text-rose-400 block">Low: {st.rbLow.toFixed(2)}</span>
                </div>
              )}
            </div>
        ...
        <div className="flex items-center justify-between pb-3 border-b border-[#1e293b]">
          <div className="flex items-center gap-2">
            ...
            <div>
              ...
              <p className="text-[10px] text-gray-400 font-mono">
                Select active execution strategies or enable multi-confluence scoring
              </p>
            </div>
          </div>
        </div>
